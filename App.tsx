...
const App: React.FC = () => {
  ...
  const trackStep = useCallback((newStep: FunnelStep) => {
    ...
    if (!currentSession) {
      currentSession = { id: sessionId.current, startTime: Date.now(), lastActive: Date.now(), stepsReached: [newStep], currentStep: newStep, deviceInfo: navigator.userAgent };
    ...
    <div className="min-h-screen bg-black overflow-hidden relative selection:bg-red-500 selection:text-white">
      {debugEnabled && (
        <div className="fixed bottom-4 left-4 z-[9999] font-mono flex flex-col gap-2">
           <button onClick={() => setStep(FunnelStep.ADMIN)} className="w-10 h-10 bg-blue-600 text-white rounded-full flex items-center justify-center shadow-lg hover:scale-110 transition-transform border border-blue-500/50" title="Panel de Administración"><BarChart3 size={20} /></button>
          {!showDevPanel ? (
            <button onClick={() => setShowDevPanel(true)} className="w-10 h-10 bg-red-600 text-white rounded-full flex items-center justify-center shadow-lg hover:scale-110 transition-transform border border-red-500/50" title="Herramientas de Desarrollo"><Settings size={20} /></button>
          ) : (
            <div className="bg-neutral-900/90 backdrop-blur-xl border border-neutral-800 rounded-2xl p-4 w-64 shadow-2xl animate-in slide-in-from-bottom-5 duration-300">
              <div className="flex items-center justify-between mb-4 border-b border-neutral-800 pb-2">
                ...
                <button onClick={() => setShowDevPanel(false)} className="text-neutral-500 hover:text-white"><X size={16} /></button>
              </div>
              <div className="flex flex-col gap-1 max-h-80 overflow-y-auto pr-2 custom-scrollbar">
                {Object.values(FunnelStep).map((s) => (
                  <button key={s} onClick={() => { setStep(s); setShowDevPanel(false); }} className={`flex items-center justify-between text-left px-3 py-2 rounded-lg text-[10px] font-bold uppercase transition-colors ${step === s ? 'bg-red-600 text-white' : 'bg-neutral-800 text-neutral-400 hover:bg-neutral-700 hover:text-neutral-200'}`}>
                    <span>{s}</span>
